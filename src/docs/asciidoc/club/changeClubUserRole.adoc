ifndef::snippets[]
:sub-snippet: ../../../../build/generated-snippets
endif::[]
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 4
:sectlinks:
:site-url: /build/asciidoc/html5/state

=== Request
include::{sub-snippet}/changeClubUserRole/http-request.adoc[]

include::{sub-snippet}/changeClubUserRole/request-fields.adoc[]

include::{sub-snippet}/changeClubUserRole/curl-request.adoc[]

include::{sub-snippet}/changeClubUserRole/request-body.adoc[]

=== Response
include::{sub-snippet}/changeClubUserRole/http-response.adoc[]

include::{sub-snippet}/changeClubUserRole/response-fields.adoc[]

include::{sub-snippet}/changeClubUserRole/response-body.adoc[]