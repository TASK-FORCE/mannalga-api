...
@RestController
@RequestMapping("club/album")
class ClubAlbumController(
        private val clubAlbumService: ClubAlbumService
) {

     @GetMapping("/{clubSeq}")
     fun gerClubAlbumList(pageable: Pageable, @PathVariable clubSeq:Long, searchOption: ClubAlbumSearchOption): ResponseDto<Page<ClubAlbumListDto>> {
          return ResponseDto(clubAlbumService.getClubAlbumList(clubSeq, searchOption, pageable))
     }

     @PostMapping
     fun registerClubAlbum(clubSeq: Long, @RequestBody body: ClubAlbumRegisterDto): ResponseDto<String> {
