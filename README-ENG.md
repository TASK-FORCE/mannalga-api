...
If the DB schema setting is complete, enter the settings of the DB you set in the [`application.properties`](https://github.com/TASK-FORCE/mannalga-api/blob/develop/src/main/resources/application.properties) file.

### Start Service
Mannalga software has backend and frontend separation.<br>
...
The Mannalga project has built, tested, and deployed pipelines all going through Github Action. We have created two versions of the action so that we can deploy both the .jar file and the container environment, which can be seen [here](https://github.com/TASK-FORCE/mannalga-api/tree/develop/.github/workflows).
